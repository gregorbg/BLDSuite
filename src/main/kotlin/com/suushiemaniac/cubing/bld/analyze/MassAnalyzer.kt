...
class MassAnalyzer(val puzzle: TwistyPuzzle, configTag: String) {
    ...
    fun <T> analyzeScrambles(scrambles: List<Algorithm>, analysisMapping: BldAnalysis.(PieceType) -> T) = this.analyzer.executionPieceTypes.associateWith { pt ->
        scrambles.map { this.analyzer.getAnalysis(it) }.map { it.analysisMapping(pt) }.groupingBy { it }.eachCount()
    }

    fun <T> analyzeScramblesFlat(scrambles: List<Algorithm>, analysisMapping: BldAnalysis.(PieceType) -> Iterable<T>) = this.analyzer.executionPieceTypes.associateWith { pt ->
        scrambles.map { this.analyzer.getAnalysis(it) }.flatMap { it.analysisMapping(pt) }.groupingBy { it }.eachCount()
    }

    fun <T> analyzeScramblesTotal(scrambles: List<Algorithm>, analysisMapping: BldAnalysis.() -> T) = scrambles.map { this.analyzer.getAnalysis(it) }.map { it.analysisMapping() }.groupingBy { it }.eachCount()
